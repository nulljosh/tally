...
## Project Overview
Multi-purpose control panel with three main features:

1. **BC Self-Serve Scraper** — Web scraper for myselfserve.gov.bc.ca (notifications, messages, payment info, service requests). Node.js + Puppeteer.
2. **DTC Navigator** — Disability Tax Credit eligibility screener and benefits guide. The core of the planned SaaS product. See PLAN.md for the full roadmap.
3. **Security Research** — Educational pentesting materials and security test tools.

**SaaS Goal:** Turn DTC Navigator into a B2B product for disability consultants and accountants. Target: $1M ARR. See `/PLAN.md` for details.

## DTC Navigator

### Key Files
- `data/dtc-knowledge.json` — Eligibility criteria, T2201 form fields, screener questions
- `web/unified.html` — "DTC Navigator" tab with eligibility screener UI
- `src/api.js` — `POST /api/dtc/screen` endpoint for eligibility calculation
- `PLAN.md` — Full product roadmap and revenue model

### How It Works
1. User answers 12 questions about their condition, daily impact, and tax history
2. Scoring algorithm calculates DTC + PWD eligibility likelihood
3. Returns: eligibility assessment, estimated refund, applicable programs, next steps
4. Currently client-side calculation with server-side API as backup

### Scoring Model
- DTC score 70+ = "Likely eligible"
- DTC score 50-69 = "Possible"
- DTC score 30-49 = "Unlikely"
- Score <30 = "No"
- Factors: diagnosis, condition type, duration, daily impact, need for help, tax history
